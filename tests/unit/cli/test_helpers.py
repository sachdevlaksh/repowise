"""Unit tests for wikicode.cli.helpers."""
...
class TestWikicodeDir:
    def test_get_wikicode_dir(self, tmp_path):
        assert get_wikicode_dir(tmp_path) == tmp_path / ".wikicode"

    def test_ensure_wikicode_dir_creates(self, tmp_path):
        d = ensure_wikicode_dir(tmp_path)
        assert d.exists()
        assert d == tmp_path / ".wikicode"

    def test_ensure_wikicode_dir_idempotent(self, tmp_path):
        ensure_wikicode_dir(tmp_path)
        d = ensure_wikicode_dir(tmp_path)
        assert d.exists()
...
class TestStateFile:
    ...
    def test_save_creates_wikicode_dir(self, tmp_path):
        save_state(tmp_path, {"key": "value"})
        assert (tmp_path / ".wikicode" / "state.json").exists()
