...
from wikicode.core.persistence.models import GenerationJob, Page
from wikicode.server import __version__
from wikicode.server.schemas import HealthResponse
    ...
    try:
        async with get_session(factory) as session:
            ...
            for status_val in ("fresh", "stale", "expired"):
                ...
                lines.append(
                    f'wikicode_pages_total{{status="{status_val}"}} {count}'
                )
            ...
            for job_status in ("pending", "running", "completed", "failed"):
                ...
                count = result.scalar() or 0
                lines.append(
                    f'wikicode_jobs_total{{status="{job_status}"}} {count}'
                )
            ...
            for token_type, col in [
                ...
                lines.append(
                    f'wikicode_tokens_total{{type="{token_type}"}} {total}'
                )
    except Exception:
        lines.append("wikicode_health 0")
