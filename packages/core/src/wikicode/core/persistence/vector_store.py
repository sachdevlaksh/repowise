"""Vector store abstraction and implementations for WikiCode semantic search.
...
LanceDBVectorStore
    ...
    ``wikicode-core[search]`` extra (lancedb>=0.12).

PgVectorStore
    Stores embeddings in the ``wiki_pages.embedding`` pgvector column.
    Requires the ``wikicode-core[pgvector]`` extra and a running PostgreSQL
...
class LanceDBVectorStore(VectorStore):
    ...
    Requires the ``wikicode-core[search]`` extra:
        pip install wikicode-core[search]

    Data is stored in *db_path* (e.g. ``.wikicode/lancedb/``).
...
class PgVectorStore(VectorStore):
    ...
    - The ``wikicode-core[pgvector]`` extra.
