"""Async database engine and session factory for WikiCode.
...
_DEFAULT_URL = "sqlite+aiosqlite:///.wikicode/wiki.db"
...
def get_db_url(raw_url: str | None = None) -> str:
    ...
    - ``None`` → default SQLite path: ``sqlite+aiosqlite:///.wikicode/wiki.db``
    """
    if raw_url is None:
        return _DEFAULT_URL
