"""File traversal for the WikiCode ingestion pipeline.
...
source file that should be documented.  It respects:
  ...
  2. .wikicodeIgnore (same syntax, user overrides)
...
class FileTraverser:
    ...
    def __init__(
        ...
        *,
        ...
        # Per-directory .wikicodeIgnore cache: absolute dir path -> PathSpec.
    ...
    def _walk(self) -> Iterator[Path]:
        ...
        for dirpath, dirnames, filenames in os.walk(self.repo_root):
            ...
            # Load per-directory .wikicodeIgnore for subdirectory pruning.
    ...
    def _get_dir_ignore(self, dirpath: Path) -> pathspec.PathSpec:
        """Return the per-directory .wikicodeIgnore spec, loading and caching on first access."""
        key = str(dirpath)
    ...
    def _build_file_info(self, abs_path: Path) -> FileInfo | None:
        ...
        if self._extra_exclude.match_file(rel_str):
            return None
        # Per-directory .wikicodeIgnore: check filename against the parent dir's spec.
