"""``wikicode watch`` — watch for file changes and auto-update."""
...
def watch_command(
    ...
    class WikiCodeHandler(FileSystemEventHandler):
        def on_any_event(self, event):
            nonlocal timer
            if event.is_directory:
                return
            # Ignore .wikicode/ directory changes
            rel = str(Path(event.src_path).relative_to(repo_path))
            if rel.startswith(".wikicode"):
                return
    ...
    observer = Observer()
    observer.schedule(WikiCodeHandler(), str(repo_path), recursive=True)
    observer.start()
