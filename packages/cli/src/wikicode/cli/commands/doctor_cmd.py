"""``wikicode doctor`` — health check for the wiki setup."""
...
def doctor_command(path: str | None) -> None:
    ...
    # 2. .wikicode/ exists?
    wikicode_dir = get_wikicode_dir(repo_path)
    checks.append(_check(".wikicode/ directory", wikicode_dir.exists(), str(wikicode_dir)))
    ...
    provider_ok = False
    try:
        from wikicode.core.providers import list_providers
    ...
    # Display
    table = Table(title="WikiCode Doctor")
