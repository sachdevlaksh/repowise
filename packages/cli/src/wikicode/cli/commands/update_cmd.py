"""``wikicode update`` — incremental wiki regeneration for changed files."""
...
def update_command(
    ...
    if base_ref is None:
        raise click.ClickException(
            "No previous sync found. Run 'wikicode init' first or pass --since."
        )
    ...
    console.print(f"[bold]WikiCode update[/bold] — {repo_path}")
    console.print(f"Diffing [cyan]{base_ref[:8]}..{(head or 'HEAD')[:8]}[/cyan]")

    from wikicode.core.ingestion import ChangeDetector
    ...
    async def _persist() -> None:
        ...
        try:
            ...
            if git_meta_map:
                from wikicode.core.persistence.crud import recompute_decision_staleness
