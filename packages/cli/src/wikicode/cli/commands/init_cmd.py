"""``wikicode init`` — full wiki generation for a repository."""
...
from wikicode.cli.cost_estimator import build_generation_plan, estimate_cost
...
async def _persist_index_only(
    ...
    from wikicode.cli.helpers import get_db_url_for_repo
    ...
    async with get_session(sf) as session:
        ...
        if decision_report and decision_report.decisions:
            ...
            from wikicode.core.persistence.crud import bulk_upsert_decisions
...
def init_command(
    ...
    if index_only:
        ...
        console.print(f"[bold]WikiCode index-only[/bold] — {repo_path}")
        ...
        console.print(f"[bold]WikiCode init[/bold] — {repo_path}")
        ...
        from wikicode.core.providers.base import ProviderError
    ...
    from wikicode.core.generation import GenerationConfig
    ...
    try:
        from wikicode.core.analysis.decision_extractor import DecisionExtractor
    ...
    # ---- Step 4: Generate ----
    from wikicode.core.generation import ContextAssembler, JobSystem, PageGenerator
    ...
    # Save config so subsequent `wikicode update / serve` picks up the same settings.
    ...
    # ---- Step 8: MCP Config ----
    from wikicode.cli.mcp_config import format_setup_instructions, save_mcp_config
