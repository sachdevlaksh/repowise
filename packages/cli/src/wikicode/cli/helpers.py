...
def get_wikicode_dir(repo_path: Path) -> Path:
    """Return the ``.wikicode/`` directory for a given repo root."""
    return repo_path / WIKICODE_DIR


def ensure_wikicode_dir(repo_path: Path) -> Path:
    """Create the ``.wikicode/`` directory if it does not exist and return it."""
    d = get_wikicode_dir(repo_path)
...
def get_db_url_for_repo(repo_path: Path) -> str:
    ...
    If ``WIKICODE_DB_URL`` is set in the environment that URL is used (allows
    multiple repos to share a single central database).  Otherwise falls back
    to the per-repo ``<repo>/.wikicode/wiki.db`` SQLite file.
    """
    env_url = os.environ.get("WIKICODE_DB_URL")
    if env_url:
        return env_url
    db_path = get_wikicode_dir(repo_path) / DB_FILENAME
...
def load_state(repo_path: Path) -> dict[str, Any]:
    """Load ``.wikicode/state.json`` or return an empty dict if absent."""
    state_path = get_wikicode_dir(repo_path) / STATE_FILENAME
...
def save_state(repo_path: Path, state: dict[str, Any]) -> None:
    """Write *state* to ``.wikicode/state.json``."""
...
def load_config(repo_path: Path) -> dict[str, Any]:
    """Load ``.wikicode/config.yaml`` or return an empty dict if absent."""
    config_path = get_wikicode_dir(repo_path) / CONFIG_FILENAME
...
def save_config(
    ...
    *,
    ...
    """Write provider/model/embedder (and optionally exclude_patterns) to ``.wikicode/config.yaml``.
...
def resolve_provider(
    ...
    Resolution order:
      ...
      2. ``WIKICODE_PROVIDER`` env var
      3. ``.wikicode/config.yaml`` (written by ``wikicode init``)
    ...
    if provider_name is None:
        provider_name = os.environ.get("WIKICODE_PROVIDER")
