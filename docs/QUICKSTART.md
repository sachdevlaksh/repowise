...
Get the WikiCode backend and frontend running locally against an already-indexed repo.
...
- A repo that has already been indexed with `wikicode init` (the `.wikicode/` directory exists)
...
Wiki DB:    C:\Users\ragha\Desktop\interview-coach\.wikicode\wiki.db
...
uv run uvicorn wikicode.server.app:create_app --factory --host 0.0.0.0 --port 7337 --reload
```
...
|----------|----------|-------------|
| `WIKICODE_DB_URL` | Yes | SQLite/PostgreSQL connection string pointing to the indexed wiki DB |
| `GEMINI_API_KEY` | If using Gemini | Google Gemini API key |
| `ANTHROPIC_API_KEY` | If using Anthropic | Anthropic API key |
| `OPENAI_API_KEY` | If using OpenAI | OpenAI API key |
| `WIKICODE_EMBEDDER` | No (default: `mock`) | Embedder for RAG search: `gemini`, `openai`, or `mock` |
| `WIKICODE_API_URL` | Frontend only | Backend URL for Next.js proxy (default: `http://localhost:7337`) |
| `WIKICODE_API_KEY` | No | Optional API key to protect the backend; omit for local dev |
...
- **"embedder.mock_active" warning** — Set `$env:WIKICODE_EMBEDDER = "gemini"` (or `openai`) for real vector search
- **Empty repo list** — Check `WIKICODE_DB_URL` points to a DB with indexed repos
